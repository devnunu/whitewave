...
class PresetViewModel(
    ...
    PresetContract.State(categories = PresetCategories.getCategoryList())
) {
    private val selectedCategoryFlow = MutableStateFlow(PresetCategories.ALL.category)
    ...
    init {
        ...
        // 초기 프리셋 로드
        handleViewEvent(PresetContract.ViewEvent.LoadPresets)
    }

    override fun handleViewEvent(viewEvent: PresetContract.ViewEvent) {
        when (viewEvent) {
            is PresetContract.ViewEvent.LoadPresets -> loadPresets()
            is PresetContract.ViewEvent.SelectCategory -> selectCategory(viewEvent.category)
            is PresetContract.ViewEvent.SavePreset -> savePreset(viewEvent.name, viewEvent.sounds, viewEvent.category)
            is PresetContract.ViewEvent.UpdatePreset -> updatePreset(
                viewEvent.presetId,
                viewEvent.name,
                viewEvent.sounds,
                viewEvent.category
            )
            is PresetContract.ViewEvent.DeletePreset -> deletePreset(viewEvent.presetId)
            is PresetContract.ViewEvent.SelectPreset -> selectPreset(viewEvent.preset)
            is PresetContract.ViewEvent.StartEditPreset -> startEditPreset(viewEvent.preset)
            is PresetContract.ViewEvent.CancelEditPreset -> cancelEditPreset()
            is PresetContract.ViewEvent.ShowSnackbarMessage -> sendEffect(PresetContract.Effect.ShowSnackbar(viewEvent.message))
            is PresetContract.ViewEvent.ShowDialog -> sendEffect(PresetContract.Effect.ShowDialog(viewEvent.title, viewEvent.message))

            // 프리미엄 관련 이벤트 처리 추가
            is PresetContract.ViewEvent.ShowPremiumDialog -> setState { it.copy(showPremiumDialog = true) }
            is PresetContract.ViewEvent.DismissPremiumDialog -> setState { it.copy(showPremiumDialog = false) }
            is PresetContract.ViewEvent.StartSubscription -> startSubscription(viewEvent.activity)
        }
    }
    ...
    private fun savePreset(name: String, sounds: List<Sound>, category: String) {
        viewModelScope.launch {
            try {
                ...
                sendEffect(PresetContract.Effect.ShowSnackbar("프리셋이 저장되었습니다"))
            } catch (e: PresetLimitExceededException) {
                setState { it.copy(error = e.message) }
                sendEffect(PresetContract.Effect.ShowSnackbar(e.message ?: "프리셋 저장 오류"))
            } catch (e: Exception) {
                setState { it.copy(error = e.message) }
                sendEffect(PresetContract.Effect.ShowSnackbar(e.message ?: "알 수 없는 오류가 발생했습니다"))
            }
        }
    }
    ...
    private fun updatePreset(
        ...
        viewModelScope.launch {
            try {
                ...
                sendEffect(PresetContract.Effect.ShowSnackbar("프리셋이 업데이트되었습니다"))
            } catch (e: Exception) {
                setState { it.copy(error = e.message) }
                sendEffect(PresetContract.Effect.ShowSnackbar(e.message ?: "프리셋 업데이트 오류"))
            }
        }
    }
    ...
    private fun deletePreset(presetId: String) {
        viewModelScope.launch {
            try {
                ...
                sendEffect(PresetContract.Effect.ShowSnackbar("프리셋이 삭제되었습니다"))
            } catch (e: DefaultPresetDeletionException) {
                sendEffect(
                    PresetContract.Effect.ShowDialog(
                    "삭제 불가",
                    "기본 프리셋은 삭제할 수 없습니다."
                ))
            } catch (e: Exception) {
                setState { it.copy(error = e.message) }
                sendEffect(PresetContract.Effect.ShowSnackbar(e.message ?: "프리셋 삭제 오류"))
            }
        }
    }
    ...
    private fun startSubscription(activity: Activity) {
        viewModelScope.launch {
            try {
                ...
                sendEffect(PresetContract.Effect.ShowSnackbar("구독 시작 중 오류가 발생했습니다: ${e.message}"))
            }
        }
    }
}